...
AI Hub에서 제공하는 아파트 도면 데이터를 활용하여 커스텀 통합 데이터셋을 구축합니다.

*   **데이터 구축 (Scrambled Hash Matching)**: 원본 데이터 배포 시 난독화(Scrambled)되어 각기 다른 폴더에 나뉘어 있던 가구 도면과 구조물 도면을 파일 크기(Size)와 CRC32 해시 대조 기법으로 병합. 이를 통해 가구와 문/창호 정답지가 모두 포함된 **약 5,000장 규모의 마스터 데이터셋**을 구축.
*   **데이터 분할**: Train : Val : Test = **8 : 1 : 1** 비율로 분할.

| 과제 | 사용 데이터 | 내용 | 활용 Phase |
|------|------|------|-----------| 
| **가구/설비/구조물** | `object_layout` & `structural_elements` | 주요 가구/설비 및 구조물 7종 (변기, 세면대, 싱크대, 욕조, 가스레인지, 문, 창호) 통합 모델 학습 | Phase 1 & 2 |
| **텍스트(OCR) 탐지** | `ocr` | 도면 내 텍스트 위치(BBox) 탐지 모델 학습 | Phase 3 |

> **경량화 아키텍처: 왜 Text Recognition 모델을 붙이지 않는가?**
> 본 프로젝트의 핵심 목표는 "도면의 공간적 레이아웃과 객체의 좌표를 JSON으로 구조화"하는 것입니다. 텍스트의 정확한 '내용'을 읽어내는 무거운 인식 엔진(PaddleOCR 등)을 붙이는 대신, **YOLO를 이용해 텍스트의 '위치(BBox)'만 탐지하여 JSON에 기록하는 경량화 앙상블 전략**을 채택했습니다. 이를 통해 파이프라인의 속도와 복잡도를 최적화합니다.
